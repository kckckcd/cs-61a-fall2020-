...
def start(file_args, port, open_browser):
    ...
    for port in range(PORT, PORT + 10):
        request = Request("http://127.0.0.1:{port}/kill", method='POST')
        try:
        ...
        try:
            ...
            print("Port {port} is currently in use, trying a different port...")
        else:
            PORT = port
        ...
        if supports_color():
            print("\033[91m", end="")
        print("Unable to connect to any candidate ports, printing debug information:")
        print(e)
        if supports_color():
            print("\033[0m", end="")
        return
    ...
    if open_browser:
        webbrowser.open("http://127.0.0.1:{PORT}", new=0, autoraise=True)
    try:
        httpd.serve_forever()
