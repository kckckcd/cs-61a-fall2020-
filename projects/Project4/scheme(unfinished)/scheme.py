...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        # BEGIN PROBLEM 10
        "*** YOUR CODE HERE ***"
        mini_frame = Frame(self)
        while formals != nil:
            mini_frame.bindings[formals.first] = vals.first
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 9
        "*** YOUR CODE HERE ***"
        forma, body, res = target.rest, expressions.rest, target.first
        env.bindings[res] = do_lambda_form(Pair(forma, body), env)
        return res
        # END PROBLEM 9
    else:
...
def do_and_form(expressions, env):
    ...
    if is_false_primitive(temp):
        return False
    elif is_true_primitive(temp) and expressions.rest == nil:
        return temp
    else:
