...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        temp_place, pace = self.place, 0
        while temp_place != beehive:
            res = rANTdom_else_none(temp_place.bees)
            if res and self.min_range <= pace <= self.max_range:
                return res
            else:
                temp_place, pace = temp_place.entrance, pace + 1
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if amount >= self.armor:
            fire_damage = self.damage + 1
        else:
            fire_damage = amount
        temp = self.place.bees
        for i in range(len(temp)-1, -1, -1):
            self.place.bees[i].reduce_armor(fire_damage)
    ...
    def action(self, gamestate):
        ...
        if self.true_queen == 0:
            ...
            while temp_place.exit != None:
                if temp_place.ant and not temp_place.ant.doubled:
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem Optional 2
        "*** YOUR CODE HERE ***"
        if not self.contained_ant and not isinstance(ContainerAnt, other):
            return True
        else:
            return False
        # END Problem Optional 2
